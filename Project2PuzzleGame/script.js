...
function startCountdown() {
    ...
    const interval = setInterval(() => {
        document.getElementById("timeSpent").innerHTML = counter;
        console.log(counter); //FOR DEBUGGING THE COUNTER
        counter++;
    }, 1000);
...
window.onload = function () {
    ...
    var newGameBtn = document.getElementById('shufflebutton'); //initializes the new game button
    spaceX = '300px';
    spaceY = '300px';

    newGameBtn.onclick = function () //activates whenever the newGameBtn button is clicked
    {
        startCountdown();
        countMoves = 0;
        for (var i = 0; i < 300; i++) {
...
function Notify() //notifies the user
{
    ...
    if (notify == 0) //if the value reaches the end then
    {
        ...
        alert('Winner! ... Press New Game and Play Again'); //tells the user that they have won the game
